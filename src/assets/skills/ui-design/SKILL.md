...
description: Design and build pages and UI for the Alenna Analytics dashboard (React, Vite, Tailwind v4, shadcn-style primitives, Clerk). Use when adding or changing routes, layout, charts, filters, forms, or visual polish. Aligns with existing tokens in src/index.css and shell copy in src/lib/i18n/shell-strings.ts.
---
...
**Structure:** See **[`src/README.md`](../../src/README.md)** for the full layout manifest (`ui`, `shell`, `pages`).
...
| App shell | `src/shell/layout/app-shell-layout.tsx` — sidebar + header + outlet |
| Auth gate | `src/shell/app-auth-boundary.tsx` |
| Sidebar / header | `src/shell/layout/app-sidebar.tsx`, `app-header.tsx`, `app-boot-loader.tsx` |
| i18n (EN/ES) | `src/lib/i18n/shell-strings.ts` — **`shellT(lang, key)`**; add keys to **both** `en` and `es` objects |
| Language | `src/shell/providers/language-provider.tsx` — `useLanguage()` |
| Theme | `src/shell/providers/theme-provider.tsx` |
...
| Integrations list | `src/pages/integrations/dashboard/IntegrationsListPage.tsx` |
| Integrations detail | `src/pages/integrations/details/` — page + colocated Shopify/placeholder views and hooks |
| Charts | **Recharts** is a dependency (`recharts`); series colors should use **`--chart-1` … `--chart-5`**, **`--chart-grid`**, etc. |

New **pages** go under `src/pages/` (grouped by area), wired in `App.tsx` (nested under `/dashboard` when authenticated).
...
2. List UI pieces; prefer **composing** `src/ui/*` (Button, Card, Input, Select, Table, Tabs, Dialog, …).
3. Any **user-visible string** → add to **`src/lib/i18n/shell-strings.ts`** (both locales); use `shellT(lang, '…')` from `useLanguage()`.
...
If a one-off primitive is truly missing, add it under `src/ui/` following existing shadcn-style patterns (Radix/Base UI as already used).
...
**Layout:** `app-sidebar`, `app-header`, `app-boot-loader`, `app-error-boundary` (under `src/shell/`).

**UI primitives (`src/ui/`):** `button`, `card`, `input`, `label`, `select`, `table`, `tabs`, `dialog`, `dropdown-menu`, `popover`, `command`, `calendar`, `badge`, `tooltip`, `skeleton`, `scroll-area`, `separator`.

**Providers:** `app-providers`, `query-provider`, `workspace-context`, `language-provider`, `theme-provider` (under `src/shell/providers/`).

**Auth:** `src/shell/auth/auth-login-page.tsx` (used from `HomePage`).

There is **no** separate `MetricBlock` / `DataTable` wrapper in this repo yet — **introduce small composed components** under `src/pages/<area>/…` or `src/ui/` when the same pattern repeats.
...
2. Route + file list (`pages/`, `ui/`, `shell-strings` keys).
3. Code.
...
- Hardcoded English/Spanish strings in components **without** `lib/i18n/shell-strings.ts`.
