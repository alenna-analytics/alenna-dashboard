...
export function useConnectorsList() {
  ...
  return useQuery<PlatformConnection[]>({
    ...
    queryFn: () => fetchJson<PlatformConnection[]>('/connectors', (a) => getToken(a)),
...
export function useShopifyAuthorizationUrl() {
  ...
  return useMutation({
    mutationFn: async (shop: string) => {
      ...
      const res = await apiFetch(`/connectors/shopify/authorization-url?${q}`, (a) => getToken(a))
      if (!res.ok) {
        const text = await res.text()
        throw new Error(text || res.statusText)
      }
...
export function useShopifyDisconnect() {
  ...
  return useMutation({
    mutationFn: async () => {
      const res = await apiFetch('/connectors/shopify', (a) => getToken(a), { method: 'DELETE' })
