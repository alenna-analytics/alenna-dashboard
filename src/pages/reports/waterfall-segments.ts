import type { ShellStringKey } from '@/lib/i18n/shell-strings'
import type { KpiResponse } from '@/lib/types/reports'
import type { PnlRowId } from '@/pages/reports/reports-pnl-rows'

import type { Segment } from './waterfall-chart'

export function buildWaterfallSegments(
  kpi: KpiResponse,
  labelForRow: (id: PnlRowId) => string,
  t: (key: ShellStringKey) => string,
): Segment[] {
  return [
    {
      name: labelForRow('gross_revenue'),
      value: kpi.gross_revenue,
      isSubtotal: true,
      isNegative: false,
    },
    {
      name: t('reportsWfDiscountsReturns'),
      value: kpi.discounts + kpi.returns,
      isSubtotal: false,
      isNegative: true,
      stackedParts: [
        { name: labelForRow('discounts'), value: kpi.discounts, isNegative: true },
        { name: labelForRow('returns'), value: kpi.returns, isNegative: true },
      ],
    },
    {
      name: labelForRow('net_revenue'),
      value: kpi.net_revenue,
      isSubtotal: true,
      isNegative: false,
    },
    { name: labelForRow('cogs'), value: kpi.cogs, isSubtotal: false, isNegative: true },
    {
      name: labelForRow('gross_profit'),
      value: kpi.gross_profit,
      isSubtotal: true,
      isNegative: false,
    },
    {
      name: labelForRow('platform_fees'),
      value: kpi.platform_fees_total,
      isSubtotal: false,
      isNegative: true,
    },
    {
      name: labelForRow('merchant_shipping'),
      value: kpi.merchant_shipping_cost,
      isSubtotal: false,
      isNegative: true,
    },
    { name: labelForRow('ads_spend'), value: kpi.ads_spend, isSubtotal: false, isNegative: true },
    {
      name: labelForRow('contribution_margin'),
      value: kpi.contribution_margin,
      isSubtotal: true,
      isNegative: kpi.contribution_margin < 0,
    },
    {
      name: labelForRow('fixed_opex'),
      value: kpi.fixed_operating_expenses,
      isSubtotal: false,
      isNegative: true,
    },
    {
      name: labelForRow('ebitda'),
      value: kpi.ebitda,
      isSubtotal: true,
      isNegative: kpi.ebitda < 0,
    },
  ]
}
