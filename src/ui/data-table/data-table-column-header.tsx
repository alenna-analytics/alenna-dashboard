...
export function DataTableColumnHeader<TData, TValue>({
  ...
  if (!column.getCanSort()) {
    return (
      <div
        className={cn(
          "flex w-full min-w-0 items-center font-numeric text-xs font-medium uppercase text-muted-foreground",
          className,
        )}
      >
        {title}
  ...
  return (
    <div className={cn("flex w-full min-w-0 items-center gap-1", className)}>
      <button
        type="button"
        className="-ml-1.5 inline-flex h-8 shrink-0 items-center gap-1 rounded-sm px-2 font-numeric text-xs font-medium uppercase whitespace-nowrap text-muted-foreground outline-none transition-colors hover:bg-muted/45 hover:text-foreground focus-visible:border focus-visible:border-border-default focus-visible:ring-2 focus-visible:ring-ring/30"
