...
function SheetOverlay({ className, ...props }: DialogPrimitive.Backdrop.Props) {
  return (
    <DialogPrimitive.Backdrop
      ...
      className={cn(
        "fixed inset-0 z-50 bg-white/40 backdrop-blur-sm supports-backdrop-filter:bg-[rgba(255,252,245,0.28)] motion-safe:duration-200 data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0",
        className,
      )}
      {...props}
    />
    ...
    <SheetPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          ...
          side === "right" &&
            "inset-y-0 right-0 w-full max-w-[var(--shell-sheet-width)] rounded-tl-xl rounded-bl-xl border-l border-[var(--shell-divider)] data-closed:slide-out-to-right data-open:slide-in-from-right",
          side === "left" &&
